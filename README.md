...
Für das Intervall $i_{groß}$ der langen Wegstrecke funktioniert das zunächst analog, wir starten zunächst mit dem Intervall von $avg$ bis $∞$. Bei einem Schritt des Algorithmus wird dann in allen Intervallen nach denjenigen gefiltert, deren mittlerer Wert größer ist als $avg$, und die bei einem Schnitt mit $i_{groß}$ kein leeres Intervall ergeben. von diesen wird dann dasjenige Intervall mit $i_{groß}$ geschnitten und als neues $i_{groß}$ abgespeichert, dessen Schnitt mit $i_{groß}$ am größten ist.
